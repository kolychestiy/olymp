...
Остаётся только вместо условного классического модуля $10^9 + 7$ использовать наш сгенерированный `mod` и заменить в хешах все `int` или `long long` на `__int128`.
